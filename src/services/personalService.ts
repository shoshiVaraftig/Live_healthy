...
export const personalService = {
  getPersonalArea: async (): Promise<PersonalArea> => {
    try {
      // הנקודה הסופית (endpoint) של ה-API שלך היא /api/User
      // בהנחה שה-backend יודע לזהות את המשתמש מהטוקן
      const data = await authService.fetchWithAuth<PersonalArea>(`${API_BASE_URL}/api/User`, {
        method: 'GET',
      });
      return data;
    } catch (error) {
  ...
  updatePersonalArea: async (userId: number, personalData: Partial<PersonalArea>): Promise<PersonalArea> => {
    try {
